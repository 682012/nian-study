    ...
    <div className="chat-mask">
      <div className="chat-dialog settings-dialog">
        <header className="chat-head">
          <div style={{ flex: 1 }}><strong>云端念安设置</strong><small>OpenAI 兼容网关 · Key 只存本机</small></div>
          <button className="quiz-close" onClick={onClose} aria-label="关闭">×</button>
        </header>
        <div className="settings-body">
          <label className="switch-row">
            <span>启用云端 AI<small>关闭时用本地规则应答，离线可用</small></span>
            <input type="checkbox" checked={draft.enabled} onChange={(e) => setDraft({ ...draft, enabled: e.target.checked })} />
          </label>
          <label>网关地址（Base URL）
            <input value={draft.baseUrl} onChange={(e) => setDraft({ ...draft, baseUrl: e.target.value })} placeholder="https://api.openai.com/v1" autoCapitalize="off" />
          </label>
          <label>模型
            <input value={draft.model} onChange={(e) => setDraft({ ...draft, model: e.target.value })} placeholder="gpt-4o-mini" autoCapitalize="off" />
          </label>
          <label>API Key
            <input value={draft.apiKey} type="password" onChange={(e) => setDraft({ ...draft, apiKey: e.target.value })} placeholder="sk-..." autoCapitalize="off" />
          </label>
